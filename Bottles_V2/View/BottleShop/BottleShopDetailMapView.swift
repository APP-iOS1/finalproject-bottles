//
//  BottleShopDetailMapView.swift
//  Bottles_V2
//
//  Created by 강창현 on 2023/02/09.
//
...
struct BottleShopDetailMapView: UIViewRepresentable {
    ...
    init(_ coord: (Double, Double), _ shopModel: ShopModel) {
        self.coord = coord
    ...
    func makeUIView(context: Context) -> some NMFNaverMapView {
        ...
        // MARK: - 마커 생성
            ...
            marker.position = NMGLatLng(
                lat: shopModel.location.latitude,
                lng: shopModel.location.longitude)
            marker.captionRequestedWidth = 100 // 마커 캡션 너비 지정
        marker.captionText = shopModel.shopName
            marker.captionMinZoom = 10
            marker.captionMaxZoom = 17
            
            // MARK: - 마커 이미지 변경
            marker.iconImage = NMFOverlayImage(name: shopModel.isRegister ? "MapMarker.fill" : "MapMarker")
